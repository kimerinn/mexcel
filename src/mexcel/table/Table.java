...
public class Table {
	...
	/**
	 * Computes table data. Cell references are being resolved and expressions
	 * computed. If expression could not be computed, cell is filled with
	 * #{error_description}
	 */
	public void computeCells() throws Exception {
		...
		for (Cell cell : cells.values()) {
			if (cell.getCellType() == Cell.CELL_TYPE.EXPRESSION_CELL) {
				try {
					expressionParser.evaluateAndStore(cell.getName(),
					    cell.getExpVal().toUpperCase().replaceAll("\\p{javaWhitespace}+", ""));
	...
	class ExpressionParser {
		...
		private int eval(ListIterator<String> expressionTokenizer) throws MicroExcelException {
			...
			case POSITIVE_NUMBER:
				...
				break;
			case CELL:
				operand2 = resolveCell(operand2Str);
				break;
			case OPERATOR:
				throw new ParserException("Not enough operands");
			default:
				throw new ParserException("Unknown token: " + operand2Str);
			}
		...
		private int resolveCell(String cellAddr) throws MicroExcelException {
			if (!cells.containsKey(cellAddr)) {
				...
				Cell resolvedCell = computedCells.containsKey(cellAddr) ? computedCells.get(cellAddr)
				    : cells.get(cellAddr);// use computed values, if possible

				switch (resolvedCell.getCellType()) {
				case TEXT_CELL:
					throw new ParserException("Can't use text in expression! Cell " + cellAddr);
				case NUMBER_CELL:
					return resolvedCell.getNumVal();
				case EXPRESSION_CELL:
					return evaluateAndStore(resolvedCell.getName(),
					    resolvedCell.getExpVal().toUpperCase().replaceAll("\\p{javaWhitespace}+", ""));
				case ERROR_CELL:
					throw new MicroExcelException("Expression error in cell " + cellAddr);
				default:
					throw new ParserException("Unknown cell type: " + resolvedCell.getCellType());
				}
			}
		}
